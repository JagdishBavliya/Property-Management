...
import { 
  updateProperty, 
  fetchPropertyById,
  selectUpdatePropertyLoading, 
  selectUpdatePropertyError,
...
import {
  BuildingOfficeIcon, MapPinIcon, CurrencyDollarIcon, UserIcon,
  PhoneIcon, DocumentIcon, VideoCameraIcon, HomeIcon,
  BuildingStorefrontIcon, CameraIcon, PencilSquareIcon,
...
export default function EditProperty() {
  ...
  const loading = useSelector(selectUpdatePropertyLoading);
  const error = useSelector(selectUpdatePropertyError);
  ...
  useEffect(() => {
    if (property && !isInitialized) {
      reset({
        ...
        property_rate: property.property_rate || '',
        seller_name: property.seller_name || '',
        seller_mobile: property.seller_mobile || '',
        buyer_name: property.buyer_name || '',
        buyer_mobile: property.buyer_mobile || '',
        seller_brokerage_type: property.seller_brokerage_type || 'Percentage',
        seller_brokerage_value: property.seller_brokerage_value || '',
        buyer_brokerage_type: property.buyer_brokerage_type || 'Flat',
        buyer_brokerage_value: property.buyer_brokerage_value || '',
        full_deal_amount: property.full_deal_amount || '',
        video_link: property.video_link || '',
        agent_code: property.agent_code || 'AGT-XXXXXX',
      });
      setIsInitialized(true);
    }
  }, [property, reset, isInitialized]);

  useEffect(() => {
    if (property && isInitialized) {
      const currentPropertyId = hashids.decode(hashid)[0];
      if (property.id !== currentPropertyId) {
        setIsInitialized(false);
      }
    }
  }, [property, hashid, isInitialized]);

  useEffect(() => {
    if (property && allAgents.length > 0 && isInitialized) {
      setValue('agent_code', property.agent_code || '');
    }
  }, [property, allAgents, isInitialized, setValue]);
  ...
  // Permission check using CheckPermission component
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <Card variant="elevated">
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Sorry, you are not authorized to edit this property</h3>
          <p className="text-gray-600 mb-6">Please contact the property owner to edit this property</p>
  ...
  if (allAgents.map(agent => agent.agent_code).includes(property?.agent_code) !== true) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card variant="elevated">
          <div className="text-center py-12">
  ...
  return (
    <CheckPermission permission="property-edit" fallback={<UnauthorizedMessage />}>
            ...
            <div className="flex items-center gap-4">
              <Breadcrumb
                items={[
                  ...
                  { label: property.property_name || 'Edit Property', icon: BuildingOfficeIcon }
                ]}
              />
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Card variant="elevated">
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {allAgents.map(agent => agent.agent_code).includes(property?.agent_code) !== true ? (
                    <div>
                      <p className="text-sm text-gray-500">You are not authorized to edit this property</p>
                    </div>
                  ):(
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <select
                      ...
                      defaultValue={property?.agent_code || ''}
                    >
                      {allAgentsLoading ? (
                  ...
                  <div className="md:col-span-2">
                    ...
                    {property?.property_image && !selectedImage && (
                      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <div className="text-sm font-medium text-gray-700">Current Property Image</div>
                            <div className="text-xs text-gray-500">Click below to upload a new image</div>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="md:col-span-2">
                    ...
                    {property?.property_brochure && !selectedFile && (
                      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-2">
                          <DocumentIcon className="h-5 w-5 text-gray-400" />
                          <span className="text-sm font-medium text-gray-700">Current Property Brochure</span>
                          <span className="text-xs text-gray-500">Click below to upload a new file</span>
                        </div>
                      </div>
                    )}
                    <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors">
